...
{
    int list_all_size;
    ifstream load_file ("course_database.txt", ifstream::in); //Read course database file
    ...
    ifstream stud_file("student_data.txt", ifstream::in); //read student data file
 ...
 * @brief 
 */
void Database::search_db() //search for specific courses in the database
{
    ...
    if(s.find("gpa") != string::npos)
    {
        ...
        cin >> low;
        while(cin.fail() || low < 0 || low > 4)
        {
            cin.clear();
            fflush(stdin);
            std::cout << "\nInvalid number.\nEnter the lower bound for the GPA range to search by: \n";
            cin >> low;
        }
        std::cout << "\nEnter the upper bound for the GPA range to search by: (lowerbound - 4.0)\n";
        cin >> up;
        while(cin.fail() || low > up || up > 4 || up < 0)
        {
            cin.clear();
            fflush(stdin);
            std::cout << "\nInvalid number.\nEnter the upper bound for the GPA range to search by: \n";
            cin >> up;
        }
        getchar();
    ...
    if(s.find("credhours") != string::npos)
    {
        ...
        while(cin.fail())
        {
            cin.clear();
            fflush(stdin);
            std::cout << "Invalid credit hour number.\nEnter credit hour amount: \n";
            std::cin >> cnum;
        }
        getchar();
