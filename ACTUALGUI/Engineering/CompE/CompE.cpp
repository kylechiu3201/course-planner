...
bool CompE:: remove_from_degree(Course c)
{
    ...
    {
        auto iter = std::find(get_gen_electives()->first.begin(), get_gen_electives()->first.end(), c);
        if(iter != get_gen_electives()->first.end())
            get_gen_electives()->first.erase(iter);
...
void CompE::add_to_degree(Course c) //checks to see if the course belongs to a category and adds it to the vector
{
    ...
    else if(Engineering::search_req2(get_pos_gen_electives(), c))
    {
        ...
        if(!Engineering::search_req(*get_gen_electives(), c)) //checks if in degree vec
        {
            get_gen_electives()->first.push_back(c);
        }
    }
...
void CompE::fill_pos_vec() //puts the courses from the text file in to the possible course vectors
{
    ...
    ifstream file(path + "/CE_Data.txt");
    if (file.fail())
    {
...
{
    string path = QApplication::applicationDirPath().toLocal8Bit().constData();
    ofstream deg_file(path + "/student_degree_data.txt", ofstream::out | ofstream::trunc); //open empty file for fresh save
...
{
    string path = QApplication::applicationDirPath().toLocal8Bit().constData();
    ifstream deg_file(path + "/student_degree_data.txt", ifstream::in);
    if(deg_file.fail())
    {
    ...
    for(int x = 0; x < course_sum; x++){
        Course temp;
        deg_file >> temp;
        major->get_gen_electives()->first.push_back(temp);
    }
    deg_file >> course_sum;
 ...
 * @param input
 */
int CompE::is_satisfied(string input) //courses that you still have left to take
{
    ...
    if(input =="F")
    {
        num_needed= get_pos_foundations().size() - get_foundations()->second;
        qDebug() << get_pos_foundations().size() <<  " " << get_foundations()->second;

    }
    if(input =="CO")
    {
        num_needed= get_pos_core().size() - get_core()->second;
    ...
    if(input =="GE")
    {
        ...
        int sum = tech_hours - get_tech_hours(get_EE_foundation()->first, get_adv_computing()->first, get_gen_electives()->first, get_capstone()->first);
        num_needed = sum;
 ...
 * @param input
 */
int CompE::satisfied(string input)  //displays courses taken
{
    int num_needed=0; //courses needed
    vector<Course> temp;
    //ToDo: add a way to select multiple/all of the categories
    ...
    {
        int sum = 0;
        for(int x = 0 ; x<get_gen_electives()->first.size(); x++)
        {
            sum += get_gen_electives()->first.at(x).get_credit_hours();
 ...
 * @return
 */
string CompE::select_group(string input) //picks out a certain category
{
